...
export function SiteHeader({ brand = "Winner IT" }: { brand?: string }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/60 bg-white/90 backdrop-blur-lg shadow-sm shadow-slate-900/[0.03]">
      <div className="mx-auto max-w-6xl px-3 sm:px-6 flex min-h-[3.5rem] sm:h-16 py-2 sm:py-0 items-center justify-between gap-2 sm:gap-4">
        {/* Logo + ชื่อแบรนด์บนมือถือ (เดิมซ่อน → ดูว่าง) */}
        <Link href="/" className="flex items-center gap-2 sm:gap-2.5 shrink-0 group min-w-0">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-brand-500 to-brand-600 text-white text-sm font-bold shadow-sm ring-2 ring-brand-500/20 group-hover:shadow-glow transition-shadow">
            W
          </span>
          <div className="min-w-0 flex flex-col leading-tight">
            <span className="font-bold text-slate-900 text-sm sm:text-base truncate">{brand}</span>
            <span className="text-[11px] text-slate-500 truncate md:hidden">
              รับซื้อไอที · ประเมินฟรี · นัดรับถึงที่
            </span>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          {/* LINE: มือถือ = ปุ่มกลม ไม่ต้องเปิดเมนู */}
          <a
            href={BUSINESS_INFO.lineUrl}
            target="_blank"
            rel="noreferrer"
            className="md:hidden flex h-10 w-10 items-center justify-center rounded-xl bg-brand-500 text-white shadow-sm hover:bg-brand-600 transition-colors"
            aria-label="แชท LINE"
          >
          ...
          <button
            ...
            className="md:hidden p-2 rounded-xl text-slate-600 hover:bg-slate-100 transition-colors border border-transparent hover:border-slate-200"
            aria-label="เปิดเมนู"
          >
            {open ? (
      ...
      {/* Mobile drawer */}
      {open && (
        <div className="md:hidden border-t border-slate-100 bg-gradient-to-b from-slate-50/80 to-white px-4 pb-5 pt-3 animate-fadeUp space-y-1 shadow-inner">
          {NAV.map((n) =>
            n.isAnchor ? (
